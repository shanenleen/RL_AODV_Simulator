...
public class RREP_Recieved extends Thread {
    ...
    public void run() {
        ...
        if (mynode.equals(rrepPacket.source)) {
            ...
            MyLogger.logger.info("----Node"+mynode+" RREP to me from "+packetWrapper.getRrepPacket().dest+" via "+packetWrapper.getRecievedFrom());


            ////////////////////////
            // have a mechanism to clear the array list
            synchronized (mynode.RREPPacketWrapperList) {
                mynode.RREPPacketWrapperList.add(this.packetWrapper);
                MyLogger.logger.info("----Added wrapper to list");
            }
            ///////////////////////
